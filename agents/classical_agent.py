...
class ClassicalAgent(BaseAgent):
    ...
    def __init__(self, state_dim, action_dim, global_state_dim, global_action_dim,
                 agent_name="classical_agent", actor_lr=1e-4, critic_lr=3e-4,
                 gamma=0.99, tau=0.01):
        """
        Args:
            state_dim: 局部状态维度（经典层状态）
            action_dim: 动作维度（经典层参数数量）
...
if __name__ == "__main__":
    ...
    state_dim = 18  # 经典层状态维度
    action_dim = 15  # 经典层动作维度
    ...
    print(f"Classical Agent created: {agent}")
