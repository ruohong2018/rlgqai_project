...
from networks.actor_network import AttentionActorNetwork
...
class QuantumAgent(BaseAgent):
    ...
    def __init__(self, state_dim, action_dim, global_state_dim, global_action_dim,
                 agent_name="quantum_agent", actor_lr=1e-4, critic_lr=3e-4,
                 gamma=0.99, tau=0.01, use_attention=True):
        """
        Args:
            state_dim: 局部状态维度（量子层状态）
            action_dim: 动作维度（量子层参数数量）
        ...
        if use_attention:
            ...
            self.actor_target = AttentionActorNetwork(
                state_dim, action_dim,
                hidden_dims=[256, 256],
                use_batch_norm=True
            ).to(self.device)
    ...
    def update(self, batch, all_agents_actions, all_agents_target_actions):
        ...
        Args:
            batch: 经验批次
                - local_states: 量子层局部状态
                - global_states: 全局状态
                - global_actions: 全局动作
                - rewards: 奖励
                - next_local_states: 下一时刻量子层局部状态
                - next_global_states: 下一时刻全局状态
                - dones: 结束标志
            all_agents_actions: 当前所有智能体的动作（用于计算策略梯度）
            all_agents_target_actions: 所有智能体目标网络的动作（用于计算目标Q值）
        """
...
if __name__ == "__main__":
    # 测试代码
    print("Testing QuantumAgent...")
    
    state_dim = 15  # 量子层状态维度
    action_dim = 18  # 量子层动作维度
    ...
    print(f"Quantum Agent created: {agent}")
    ...
    # 测试带噪声的动作
    noise = np.random.randn(action_dim) * 0.1
    action_with_noise = agent.get_action(state, noise)
    print(f"Action with noise: range [{action_with_noise.min():.3f}, {action_with_noise.max():.3f}]")
    
    # 测试目标动作
    target_action = agent.get_target_action(state)
    print(f"Target action shape: {target_action.shape}")
    
    print("\nQuantumAgent test passed!")
